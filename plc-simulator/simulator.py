...
async def main():
    """Точка входа"""
    # Путь к конфигу можно передать аргументом:
    #   python simulator.py config/replay_config.yaml
    config_path = sys.argv[1] if len(sys.argv) > 1 else "config/plc_config.yaml"
    app = SimulatorApplication(config_path=config_path)
    await app.run()
